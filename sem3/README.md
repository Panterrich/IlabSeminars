...
Также переменные можно передавать через аргументы команды make. Например:

```
$ make A=abs B=123
```

В таком случае make считает, как будто такая переменная была создана в начале Makefile.

В переменную можно дописать значения при помощи `+=`.
...
Напишем общий рецепт для сборки всех объектников
...
Проблема: объектные файлы начнут создаваться внутри папок. Временно сделаем по другому:
...
> ```
> Сам по себе wildcard не плох, но плохо делать своими таргетами весь мусор, который wildcard найдет в директории `src`.
...
Отлично, пропишем включение файлов зависимостей автоматически при сборке объектного файла при помощи чуть других опций компилятора:
